...
const EditAssetPurchaseOrder = () => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        Object.keys(payload).forEach((key) => {
          if (JSON.stringify(data[key]) !== JSON.stringify(payload[key])) {
            changedFields[key] = payload[key];
          }
        });

        if (Object.keys(changedFields).length > 0) {
          const response = await updateAssetPurchaseOrder(id, changedFields);
          if (response) {
            toast.success('Asset purchase order updated');
            navigate(`/${userType}/assetpurchaseorder`);
          } else {
            toast.error('Failed to update asset purchase order');
          }
        } else {
  ...
  const fetchIndentData = async () => {
    try {
      const response = await searchAssetIndent('');
      if (response) {
  ...
  return (
    ...
    >
      ...
      <ParentCard title="Edit Asset Purchase Order">
          ...
          <Grid2 container rowSpacing={2}>
            ...
            <Grid2 size={{ xs: 12, md: 9 }}>
              <Autocomplete
                ...
                getOptionLabel={(option) => {
                  ...
                  return option.indentId || '';
                }}
                value={
                  assetIndentData.find((item) => item._id === formik.values.indentId) ||
                  (data.indentId ? data.indentId : null)
                }
                onChange={(event, newValue) => {
                  ...
                  const transformedItems =
                    newValue?.items?.map((item) => ({
                      ...
                      // Preserve the populated data structure
                      codeDetails: item.itemDetails || null,
                      // Map the mainAssetId from indent search to display
                      mainAssetId:
                        item.mainAssetId ||
            ...
            <Grid2 size={{ xs: 12, md: 9 }}>
              <Autocomplete
                ...
                value={
                  vendorData.find((item) => item._id === formik.values.vendorId) ||
              ...
              <>
                ...
                <Grid2 size={{ xs: 12, md: 9 }}>
                  <CustomTextField
                    ...
                    id="authorizedBy"
                    name="authorizedBy"
                    value={data.authorizedBy?.fullName || ''}
...
export default EditAssetPurchaseOrder;
