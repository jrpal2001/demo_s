...
import { Box, Button, Grid2, MenuItem, CircularProgress, Alert } from "@mui/material"
...
import { getAssetVendorById, updateAssetVendorData } from "../../api/assetvendor"
import { useSelector } from "react-redux"
import { selectCurrentUserType } from "@/store/auth/AuthSlice"

const AssetVendorEdit = () => {
  const userType = useSelector(selectCurrentUserType)
  const BCrumb = [
    { to: "/", title: "Home" },
    { to: `/${userType}/assetvendor`, title: "Asset Vendors" },
    { title: "Edit" },
  ]
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      msmeCertificate: Yup.mixed().when("businessType", {
        ...
        then: () =>
          Yup.mixed()
            .test("fileFormat", "Unsupported file format. Only PDF, PNG, and JPEG are allowed", (value) => {
      ...
      productOrServiceDescription: Yup.string().required("Product / Service Description is required"),
      vendorPriority: Yup.string().required("Vendor Priority is required"),
      vendorProductDetails: Yup.string().required("Vendor product details is required"),
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Remove _id and __v from values before submission
        const { _id, __v, msmeCertificate, ...valuesWithoutId } = values
        console.log("Submitting vendor data:", valuesWithoutId)
        ...
        const response = await updateAssetVendorData(id, formData)
        if (response) {
          toast.success("Asset vendor updated successfully")
          navigate(`/${userType}/assetvendor`)
        } else {
          toast.error("Failed to update asset vendor")
        }
      } catch (error) {
        ...
        if (error.response?.data?.message) {
          ...
          toast.error("Failed to update asset vendor: " + (error.message || "Unknown error"))
        }
      } finally {
  ...
  if (loading && !formik.values.vendorId) {
    return (
      <PageContainer title="Admin - Edit Asset Vendor" description="Edit asset vendor details">
        <Breadcrumb title="Edit Asset Vendor" items={BCrumb} />
  ...
  return (
    <PageContainer title="Admin - Edit Asset Vendor" description="Edit asset vendor details">
      <Breadcrumb title="Edit Asset Vendor" items={BCrumb} />
      <ParentCard title="Edit Asset Vendor">
        <form
          method="POST"
          onSubmit={(e) => {
            e.preventDefault()
        ...
        >
          <Grid2 container rowSpacing={2}>
                ...
                <Grid2 size={{ xs: 12, md: 9 }}>
                  <Box>
                    ...
                    <p style={{ fontSize: "0.75rem", color: "#666", marginTop: "4px" }}>
                      Supported formats: PDF, PNG, JPEG (Max size: 5MB) - Leave empty to keep existing certificate
                    </p>
                  </Box>
                </Grid2>
              </>
            ...
            {/* Continue with remaining fields - ADDRESS through TERMS AND CONDITIONS */}
            {/* ADDRESS */}
            <Grid2 size={{ xs: 12, md: 3 }} sx={{ display: "flex", margin: 0, alignItems: "center" }}>
            ...
            <Grid2 size={{ xs: 12, md: 9 }}>
              <CustomTextField
                ...
                error={formik.touched.productOrServiceDescription && Boolean(formik.errors.productOrServiceDescription)}
                helperText={formik.touched.productOrServiceDescription && formik.errors.productOrServiceDescription}
              />
            </Grid2>
            ...
            <Grid2 size={{ xs: 12, md: 9 }}>
              <CustomSelect
                ...
                onChange={formik.handleChange}
                error={formik.touched.vendorProductDetails && Boolean(formik.errors.vendorProductDetails)}
              >
                <MenuItem value="" disabled>
          ...
          >
            <Button type="submit" variant="contained" color="primary" disabled={isSubmitting}>
              {isSubmitting ? "Updating..." : "Update Asset Vendor"}
            </Button>
            <Button type="button" variant="outlined" onClick={handleClickCancel} disabled={isSubmitting}>
              Cancel
            </Button>
          </Box>
        </form>
...
export default AssetVendorEdit
