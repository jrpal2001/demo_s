...
import { storeAssetMaterialInwardQc } from '@/api/assetMaterialInwardQc.api';
import { useSelector } from 'react-redux';
import { selectCurrentUserType } from '@/store/auth/AuthSlice';

const CreateAssetMaterialInwardQc = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const inwardData = {
          purchaseOrderNumber: purchaseOrderData?._id || '',
          invoiceNumber: values.invoiceNumber,
          lotNumber: values.lotNumber,
          debitNote: values.debitNote,
          remark: values.remark,
          items: [
            JSON.stringify({
              code: selectedItem?.code || values.itemCode, // Use the actual ObjectId
              model: selectedItem?.model || 'Asset',
              description: selectedItem?.description || values.itemDescription,
              quantityReceived: values.quantityReceived,
              quantityAccepted: values.quantityAccepted,
              uom: selectedItem?.uom || values.uom,
            }),
          ],
        ...
        console.log('Sending data to backend:', inwardData);
        const response = await storeAssetMaterialInwardQc(inwardData);
        if (response) {
          toast.success('Asset Material Inward QC created successfully');
          navigate(`/${userType}/asset-material-inward-qc`);
        }
      } catch (error) {
        toast.error(
          'Failed to create Asset Material Inward QC: ' + (error.message || 'Unknown error'),
        );
        console.error('Error creating asset material inward QC:', error);
      }
    },
  });
  ...
  useEffect(() => {
    if (purchaseOrderData && purchaseOrderData.items) {
      const items = purchaseOrderData.items.map((item) => ({
        code: item.code?._id || item.code, // Handle both populated and non-populated codes
        model: item.model,
        description: item.description,
        uom: item.uom,
        quantity: item.orderQuantity || item.quantity, // Use orderQuantity from PO
        codeDetails: item.codeDetails || item.code, // Use existing codeDetails or populated code
        displayCode: item.displayCode || getItemDisplayCode(item), // Use existing displayCode or compute
      }));
  ...
  return (
    ...
    >
      ...
      <ParentCard title="Create Asset Material Inward QC">
...
export default CreateAssetMaterialInwardQc;
