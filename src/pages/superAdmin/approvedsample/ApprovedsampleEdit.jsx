...
import { getApprovedSampleById, updateApprovedSample } from '@/api/approvedsample.api';
import { useSelector } from 'react-redux';
import { selectCurrentUserType } from '@/store/auth/AuthSlice';

const ApprovedSampleEdit = () => {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      if (!values.jobCardId || !values.productId || !values.productName || !values.designId) {
      ...
      if (hasChanges) {
        const response = await updateApprovedSample(id, updatedData);
        if (response) {
          toast.success('Approved sample updated successfully');
      ...
      toast.error('Updating approved sample failed');
    }
  };
  ...
  const formik = useFormik({
    ...
    validateOnBlur: false, // Only validate on submit
    validationSchema: Yup.object({
      ...
      sampleType: Yup.string().notOneOf(['default'], 'Please select a sample type'),
      targetMarket: Yup.string().notOneOf(['default'], 'Please select a target market'),
      designStatus: Yup.string().notOneOf(['default'], 'Please select a design status'),
      approvedStatus: Yup.string().notOneOf(['default'], 'Please select an approved status'),
    }),
    onSubmit,
  });
  ...
  const fetchData = async () => {
    try {
      const response = await getApprovedSampleById(id);
      if (response) {
        ...
        const formValues = {
          ...
          sampleType: response.sampleType || 'default',
          productDescription: response.productDescription || '',
          materialsUsed: response.materialsUsed || '',
          targetMarket: response.targetMarket || 'default',
          photoshootSample: Boolean(response.photoshootSample),
          materialUsed: response.materialUsed || '',
          designCreatedBy: response.designCreatedBy || '',
          designStatus: response.designStatus || 'default',
          approvedStatus: response.approvedStatus || 'default',
          customerReference: response.customerReference || '',
          sampleProduced: Boolean(response.sampleProduced),
          feedbackOnDesign: response.feedbackOnDesign || '',
          mannequinShootOrTableShoot: Boolean(response.mannequinShootOrTableShoot),
        };
      ...
      toast.error('Failed to fetch approved sample');
    }
  };
  ...
  return (
    <PageContainer title="Admin - Approved Sample" description="">
      ...
      <ParentCard title="Edit Approved Sample">
...
export default ApprovedSampleEdit;
