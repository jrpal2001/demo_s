  ...
  return (
    <Accordion elevation={9} sx={{ mb: 2 }} expanded={expanded} onChange={handleClick}>
      ...
      <AccordionDetails>
        ...
        {title === 'Trims' && (
          <Box sx={{ mb: 2 }}>
            {/* Flat list of all trims, add and remove, no grouping by type */}
              ...
              <Fab
                ...
                onClick={() => {
                  const currentTrims = Array.isArray(formik.values.trims)
                    ? [...formik.values.trims]
                    : [];
                  formik.setFieldValue('trims', [
                    ...currentTrims,
                    { code: '', consumption: '', uom: '', cost: '' },
                  ]);
                }}
                sx={{ marginRight: '1rem' }}
              >
            ...
            {Array.isArray(formik.values.trims) && formik.values.trims.length > 0 && (
              <Box>
                {formik.values.trims.map((trim, idx) => (
                  <Box
                    key={idx}
                    sx={{
                      position: 'relative',
                  ...
                  >
                    <ProductBoms title="Trims" formik={formik} index={idx} />
                    <Fab
                      size="small"
                      color="error"
