...
import { Autocomplete, Box, Button, Grid2, Typography, Paper, Divider } from '@mui/material';
...
import PurchaseItem from '../purchaseindent/components/PurchaseItem';

import { searchPurchaseIndent } from '@/api/purchaseindent.api';
import { searchByVendorId } from '@/api/vendor.api';
import { useNavigate } from 'react-router-dom';
import { storePurchaseOrder } from '@/api/purchaseorder.api';
...
const CreatePurchaseOrder = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        // Map items to backend-required structure
        ...
        const response = await storePurchaseOrder(payload);
        if (response) {
  ...
  const fetchIndentData = async (searchParams) => {
    try {
      const response = await searchPurchaseIndent(searchParams);
      if (response) {
  ...
  const fetchVendorData = async () => {
    try {
      const response = await searchByVendorId();
      if (response) {
  ...
  // Auto-fill suggested total when calculation changes
  ...
  return (
    <PageContainer title="Admin - Purchase Order" description="This is the purchase order page">
      ...
      <ParentCard title="Create Purchase Order">
          ...
          <Grid2 container rowSpacing={2}>
            ...
            {formik.values.items &&
              formik.values.items?.map((item, index) => {
                return <PurchaseItem formik={formik} index={index} key={index} disable={true} />;
              })}
            ...
            <Grid2 size={{ xs: 12, md: 9 }}>
              {/* Suggested Total Display */}
              {calculatedTotals.suggestedTotal > 0 && (
                <Box sx={{ mb: 1 }}>
                  <Typography variant="body2" sx={{ color: 'success.main', fontWeight: 'bold' }}>
                    💡 Calculated Total: ₹{calculatedTotals.suggestedTotal.toFixed(2)}
                    <Button
                      variant="text"
                      size="small"
                      onClick={handleUseSuggestedTotal}
