...
import { fetchAssetInventoryById, updateAssetInventory } from '@/api/assetinventory.api';
import { useSelector } from 'react-redux';
import { selectCurrentUserType } from '@/store/auth/AuthSlice';

const AssetInventoryEdit = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await updateAssetInventory(id, formData);
        if (response) {
          toast.success('Asset inventory item updated successfully');
          navigate(`/${userType}/asset-inventory`);
        }
      } catch (error) {
        console.error('🚀 ~ Update error:', error);
        toast.error('Failed to update asset inventory item: ' + (error.message || 'Unknown error'));
      } finally {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response && response.data) {
          ...
          toast.warning('Asset inventory item not found');
          navigate(`/${userType}/asset-inventory`);
        }
      } catch (error) {
        console.error('🚀 ~ fetchData error:', error);
        toast.error('Failed to fetch asset inventory item');
        navigate(`/${userType}/asset-inventory`);
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <PageContainer
        title="Admin - Edit Asset Inventory"
        description="Loading asset inventory item"
      >
        <Box
  ...
  return (
    <PageContainer title="Admin - Edit Asset Inventory" description="Edit asset inventory item">
      <Breadcrumb
        title={`Edit ${
        ...
        items={BCrumb}
      />
      <ParentCard title="Edit Asset Inventory Item">
        <form onSubmit={formik.handleSubmit}>
          ...
          >
            <Button type="submit" sx={{ marginRight: '0.5rem' }} disabled={isUploading}>
              {isUploading ? 'Updating...' : 'Update Asset Inventory'}
            </Button>
