...
import { createMaintenanceInventory } from '@/api/maintenanceInventory.api';
import { useSelector } from 'react-redux';
import { selectCurrentUserType } from '@/store/auth/AuthSlice';
import { getMaintenanceByType } from '@/api/assetmanagementERP';

const CreateMaintenanceInventory = () => {
  ...
  const [loadingMaintenanceCodes, setLoadingMaintenanceCodes] = useState(false);
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      maintenanceCode: Yup.string().required('Maintenance Code is required'),
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Add maintenance inventory data
        Object.keys(maintenanceInventoryData).forEach((key) => {
          if (key === 'lots') {
            formData.append(key, JSON.stringify(maintenanceInventoryData[key]));
          } else {
            formData.append(key, maintenanceInventoryData[key]);
          }
        });
        ...
        console.log('Sending to API:', maintenanceInventoryData);
        const response = await createMaintenanceInventory(formData);
        if (response) {
          toast.success('Maintenance Inventory created successfully');
          navigate(`/${userType}/maintenance-inventory`);
        }
      } catch (error) {
        toast.error(
          'Failed to create Maintenance Inventory: ' + (error.message || 'Unknown error'),
        );
        console.error('Error creating maintenance inventory:', error);
      } finally {
  ...
  const handleMaintenanceTypeChange = async (event) => {
    const maintenanceType = event.target.value;
    setSelectedMaintenanceType(maintenanceType);
    formik.setFieldValue('maintenanceType', maintenanceType);
    formik.setFieldValue('maintenanceCode', ''); // Reset code when type changes
    formik.setFieldValue('maintenanceName', ''); // Reset name when type changes
    formik.setFieldValue('uom', ''); // Reset UOM when type changes
    formik.setFieldValue('unitPrice', 0); // Reset unit price
    formik.setFieldValue('department', 'General'); // Reset department

    if (maintenanceType) {
      await fetchMaintenanceCodesByType(maintenanceType);
    } else {
      setMaintenanceCodes([]);
    }
  };

  const fetchMaintenanceCodesByType = async (maintenanceType) => {
    try {
      setLoadingMaintenanceCodes(true);
      const response = await getMaintenanceByType(maintenanceType);
      console.log('🚀 ~ fetchMaintenanceCodesByType ~ response:', response);

      // Handle the response structure - assuming it returns { data: [...] } or directly [...]
      const maintenanceData = response?.data?.maintenanceItems || response;

      if (maintenanceData && Array.isArray(maintenanceData)) {
        setMaintenanceCodes(maintenanceData);
      } else {
        setMaintenanceCodes([]);
        toast.warning(`No maintenance codes found for ${maintenanceType}`);
      }
    } catch (error) {
      console.error('Error fetching maintenance codes:', error);
      toast.error('Failed to fetch maintenance codes');
      setMaintenanceCodes([]);
    } finally {
      setLoadingMaintenanceCodes(false);
    }
  };

  const handleMaintenanceCodeChange = (event, newValue) => {
    console.log('Selected maintenance item:', newValue);
    if (newValue) {
      formik.setFieldValue('maintenanceCode', newValue.mainMaintenanceId);
      console.log('Setting maintenanceCode to:', newValue.mainMaintenanceId);
      // Auto-fill fields if available
      if (newValue.mainMaintenanceId) {
        formik.setFieldValue('maintenanceName', newValue.mainMaintenanceId);
      }
      if (newValue.department) {
        formik.setFieldValue('department', newValue.department);
      }
      if (newValue.unitPrice) {
        formik.setFieldValue('unitPrice', newValue.unitPrice);
      }
      // Assuming UOM might also come from ERP
      if (newValue.uom) {
        formik.setFieldValue('uom', newValue.uom);
      }
    } else {
      formik.setFieldValue('maintenanceCode', '');
      formik.setFieldValue('maintenanceName', '');
  ...
  return (
    <PageContainer
      title="Admin - Create Maintenance Inventory"
      description="Create new maintenance inventory item"
    >
      <Breadcrumb title="Create Maintenance Inventory Item" items={BCrumb} />
      <ParentCard title="Create New Maintenance Inventory Item">
        <form onSubmit={formik.handleSubmit}>
          <Grid2 container rowSpacing={2}>
            ...
            <Grid2 size={{ xs: 12, md: 9 }}>
              <Autocomplete
                ...
                autoHighlight
                loading={loadingMaintenanceCodes}
                disabled={!selectedMaintenanceType || loadingMaintenanceCodes}
                getOptionLabel={(option) => option.mainMaintenanceId || ''}
                value={
                  maintenanceCodes.find(
                    (item) => item.mainMaintenanceId === formik.values.maintenanceCode,
                  ) || null
                }
                onChange={handleMaintenanceCodeChange}
                renderInput={(params) => (
                  <CustomTextField
                    {...params}
                    placeholder={
                      !selectedMaintenanceType
                        ? 'Please select a maintenance type first'
                        : loadingMaintenanceCodes
                        ? 'Loading maintenance codes...'
                        : 'Search Maintenance Code'
                    }
                    error={formik.touched.maintenanceCode && Boolean(formik.errors.maintenanceCode)}
                ...
                renderOption={(props, option) => (
                  <Box component="li" {...props}>
                    <Box>
                      <Box sx={{ fontWeight: 'bold' }}>{option.mainMaintenanceId}</Box>
                      {option.department && (
          ...
          >
            <Button type="submit" sx={{ marginRight: '0.5rem' }} disabled={isUploading}>
              {isUploading ? 'Creating...' : 'Create Maintenance Inventory'}
            </Button>
...
export default CreateMaintenanceInventory;
