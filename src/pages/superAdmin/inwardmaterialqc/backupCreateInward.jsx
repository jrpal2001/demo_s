...
const CreateInwardMaterialQc = () => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      reportAttachment: null, // Add this new field
    },
    onSubmit: async (values) => {
      ...
      try {
        ...
        Object.entries(values).forEach(([key, value]) => {
          if (key === 'items') {
            if (Array.isArray(value) && value.length > 0) {
              // Only keep allowed fields for the single item
              const { code, quantityReceived, quantityAccepted } = value[0];
              const itemObj = {
                code: typeof code === 'object' ? code._id : code,
                quantityReceived,
                quantityAccepted,
              };
              const stringifiedItem = JSON.stringify(itemObj);
              formData.append('items', stringifiedItem);
              console.log('Appended to FormData: items =', stringifiedItem);
            }
          } else if (key === 'images') {
            // Handle multiple images - append files directly to FormData
            if (Array.isArray(value)) {
              value.forEach((file) => {
                if (file instanceof File) {
                  console.log('🚀 ~ Appending file to FormData:', file.name, file.size);
                  formData.append('images', file); // This should match the multer field name
                  console.log('Appended to FormData: images =', file.name);
                }
              });
            }
            // Don't append the images array as a regular field
            return;
          } else {
        ...
        if (response) {
          ...
          resetImages(); // Clear images after successful submission
            ...
            {purchaseOrdersData.find(
              ...
              <>
                ...
                <Grid2
                  ...
                  sx={{ display: 'flex', margin: 0, alignItems: 'center' }}
                >
                  <CustomFormLabel htmlFor="selectedItem" sx={{ marginTop: 0 }}>
                    Select Item
                  </CustomFormLabel>
                </Grid2>
                <Grid2 size={{ xs: 12, md: 9 }}>
                  <Autocomplete
                    options={purchaseOrderData.items}
                    getOptionLabel={(option) =>
                      option.code?.bomId ||
                      option.code?.fabricName ||
                      option.code?.trimsName ||
                      option.code?.accessoriesName ||
                      option.description ||
                      ''
                    }
                    value={selectedItem}
                    onChange={(event, newValue) => {
                      ...
                      formik.setFieldValue('items', [
                        {
                          code: newValue?.code?._id || newValue?.code || '',
                          quantityReceived: 0,
                          quantityAccepted: 0,
                          description: newValue?.description || '',
                          uom: newValue?.uom || '',
                          indentQuantity: newValue?.indentQuantity || 0,
                          orderQuantity: newValue?.orderQuantity || 0,
                        },
                      ]);
                    }}
                    renderInput={(params) => (
                ...
                {/* RENDER QC FIELDS FOR SELECTED ITEM */}
                {selectedItem && formik.values.items && formik.values.items.length === 1 && (
                  <InwardItemQc
                    item={selectedItem}
                    formik={formik}
                    index={0}
                    key={selectedItem.code?._id || selectedItem.code}
                  />
                )}

                {/* MULTIPLE IMAGE UPLOAD - MOVED INSIDE THE CONDITIONAL BLOCK */}
                <MultipleImageUpload
                  label="QC Images"
                  id="images"
                  onChange={addImages}
