...
import {
  getProductStyleCategoryById,
  updateProductStyleCategory,
...
const ProductStyleCategoryEdit = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      setFetching(true);
      try {
        const data = await getProductStyleCategoryById(id);
        setForm({ ...data });
        setSizeChartFileName(data.sizeChartUrl ? data.sizeChartUrl.split('/').pop() : '');
        setSpecificationsFileName(
          data.specificationsUrl ? data.specificationsUrl.split('/').pop() : '',
        );
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateProductStyleCategory(id, formData);
      toast.success('Product Style Category updated successfully');
  ...
  return (
    <PageContainer title="Edit Product Style Category" description="">
      <Breadcrumb title="Product Style Category" items={BCrumb} />
      <ParentCard title="Edit Product Style Category">
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              ...
              {form.sizeChartUrl && !sizeChartFile && (
                <Box sx={{ mb: 1 }}>
                  <a href={form.sizeChartUrl} target="_blank" rel="noopener noreferrer">
                    {sizeChartFileName || 'View Size Chart'}
                  </a>
                </Box>
              )}
              ...
              {sizeChartFileName && sizeChartFile && (
            ...
            <Grid item xs={12} md={6}>
              ...
              {form.specificationsUrl && !specificationsFile && (
                <Box sx={{ mb: 1 }}>
                  <a href={form.specificationsUrl} target="_blank" rel="noopener noreferrer">
                    {specificationsFileName || 'View Specifications'}
                  </a>
                </Box>
              )}
              <Button variant="outlined" component="label">
              ...
              {specificationsFileName && specificationsFile && (
                <Box sx={{ mt: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
...
export default ProductStyleCategoryEdit;
